...
for FILE in $PROBLEMS; do
  ...
  timeout --signal 2 --foreground 180s python -O $DIR/experiments/main.py --domain=$DOMAIN --algorithm adaptive --mode normal --logpath $LOGDIR --problem-file $FILE | tee ./save/$RUN.log
  ...
  timeout --signal 2 --foreground 180s python -O $DIR/experiments/main.py --domain=$DOMAIN --algorithm adaptive --mode oracle --oracle-options=$JSON --logpath $LOGDIR --problem-file $FILE | tee ./oracle/$RUN.log
done