...
def construct_problem_from_sim(simulator, stations, problem_info):
    ...
    def check_safe_free(arm_name, q, fluents=[]):

        fluents = fluents.copy()
        for (_, block, X_WB) in fluents:
            if not check_colfree_block(arm_name, q, block, X_WB):
                lprint(
                    f"{Colors.RED}Found collisions between {arm_name} and {block}{Colors.RESET}"
                )
                return False
        return True

    def check_safe_holding(arm_name, q, holding_block, X_HB, fluents=[]):
        # TODO: maybe do something with the fact that we're holding the block? 
        fluents = fluents.copy()
        for (_, block, X_WB) in fluents:
    ...
    stream_map = {
        ...
        "check-safe-free": from_test(check_safe_free),
        "check-safe-holding": from_test(check_safe_holding),
    }
...
if __name__ == '__main__':
    ...
    res, _ = run_blocks_world(
        problem_file="/home/mohammed/drake-tamp/experiments/blocks_world/data_generation/non_monotonic/train/1_1_1_0.yaml",
        mode="normal",
        # url = url,
        # simulate = True,
        max_time = 180
